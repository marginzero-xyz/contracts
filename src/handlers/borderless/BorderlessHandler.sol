...
/// @author arcwardeth
/// @notice Handles Borderless specific operations
/// @dev Inherits from V3BaseHandlerKodiak and LiquidityManager
contract BorderlessHandler is V3BaseHandlerKodiak, LiquidityManager {
    /// @notice Constructs the UniswapV3Handler contract
    /// @param _owner Address of the contract owner
    /// @param _feeReceiver Address to receive fees
    /// @param _factory Address of the Borderless factory
    ...
    /// @notice Adds liquidity to a Borderless pool
    ...
    function _addLiquidity(
        ...
        if (!self) {
            ...
            (l, a0, a1,) = BorderlessHandler(address(this)).addLiquidity(
                LiquidityManager.AddLiquidityParams({
                    token0: tki.token0,
                    token1: tki.token1,
                    fee: tki.fee,
    ...
    /// @notice Removes liquidity from a Borderless pool
    /// @dev Overrides the _removeLiquidity function from V3BaseHandler
    /// @param _pool The Borderless pool
    /// @param tickLower The lower tick of the position
    /// @param tickUpper The upper tick of the position
