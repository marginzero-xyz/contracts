...
abstract contract LiquidityManager is IBubblySwapMintCallback {
    ...
    function BubblySwapMintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external override {
    ...
    function computeAddress(address _factory, PoolKey memory key) internal view returns (address pool) {
        ...
        pool = IBubblySwapFactory(_factory).getPool(key.token0, key.token1, key.fee);
    }
