...
import {IBubblySwapPool} from "./IBubblySwapPool.sol";
...
abstract contract LiquidityManager is IBubblySwapMintCallback {
        ...
        )
    {
        ...
        pool = IBubblySwapPool(computeAddress(factory, poolKey));
    ...
    function computeAddress(
        ...
        pool = IBubblySwapFactory(_factory).getPool(
            key.token0,
            key.token1,
            key.fee
        );
    ...
    function verifyCallback(
        ...
        address tokenB,
        uint24 fee
    ) internal view returns (IBubblySwapPool pool) {
    ...
    function verifyCallback(
        ...
        pool = IBubblySwapPool(computeAddress(_factory, poolKey));
