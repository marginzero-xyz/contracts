...
contract UniswapV3HandlerTest is Test, UniswapV3FactoryDeployer {
    ...
    function testSplitPositionZero() public {
        ...
        for (uint256 i = 0; i < splitTicks.length - 1; i++) {
            (uint256 liquidity,,,,) =
                pool.positions(keccak256(abi.encodePacked(address(handler), splitTicks[i], splitTicks[i + 1])));
...

        {
            ...
            (liquidity,,,,) =
                pool.positions(keccak256(abi.encodePacked(address(handler), initialTickLower, initialTickUpper)));
            console.log(liquidity);
        }
